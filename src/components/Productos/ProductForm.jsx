      ...
      <form onSubmit={handleSubmit} className="modern-form-body">
            ...
            {!showNewCategory ? (
              ...
              <div className="new-category-input">
                <input
                  ...
                  autoFocus
                />
                <button type="button" onClick={() => { setShowNewCategory(false); setNewCategory(''); }} className="cancel-btn">Cancelar</button>
              </div>
            )}
          </div>
        ...
        <div className="modern-form-row">
          <div className="modern-form-group">
            ...
            <div className="modern-input-wrapper">
              <span className="input-icon">💰</span>
              <input type="number" step="0.01" value={formData.precio_compra} onChange={(e) => setFormData({ ...formData, precio_compra: parseFloat(e.target.value) || 0 })} />
            </div>
          </div>
          <div className="modern-form-group">
            ...
            <div className="modern-input-wrapper">
              <span className="input-icon">💵</span>
              <input type="number" step="0.01" value={formData.precio_venta} onChange={(e) => setFormData({ ...formData, precio_venta: parseFloat(e.target.value) || 0 })} className={errors.precio_venta ? 'error' : ''} />
            </div>
        ...
        <div className="modern-form-row">
          ...
          <div className="modern-form-group">
            ...
            <div className="modern-input-wrapper">
              <span className="input-icon">⚠️</span>
              <input type="number" min="0" value={formData.stock_min} onChange={(e) => setFormData({ ...formData, stock_min: parseInt(e.target.value) || 0 })} className={errors.stock_min ? 'error' : ''} />
            </div>
        ...
        <div className="modern-form-group">
          ...
          <div className="toggle-switch">
            <button type="button" className={`toggle-option ${formData.estado === 'activo' ? 'active' : ''}`} onClick={() => setFormData({ ...formData, estado: 'activo' })}>✅ Activo</button>
            <button type="button" className={`toggle-option ${formData.estado === 'inactivo' ? 'active' : ''}`} onClick={() => setFormData({ ...formData, estado: 'inactivo' })}>⛔ Inactivo</button>
          </div>
        </div>

        <div className="modern-form-actions">
          <button type="submit" className="btn-modern-primary" disabled={loading}>{loading ? 'Guardando...' : (editProduct ? '💾 Actualizar Producto' : '✨ Guardar Producto')}</button>
          {editProduct && (<button type="button" className="btn-modern-secondary" onClick={onCancel}>Cancelar</button>)}
        </div>
      </form>
    </div>
  );
